...
fn mpa_str(folder: &std::path::PathBuf) -> core::result::Result<String, std::io::Error> { Ok(serde_json::to_string(&pack(&folder)?)?) }
...
fn roundtrip_t() -> core::result::Result<(), std::io::Error> { tokio::runtime::Runtime::new().unwrap().block_on(async {
    ...
    let json_file = cwd().join(format!("tests/{}.mpa", "ExampleProject")); // .mpa - "Marko plain-text archive"
    let script_project: marko_fs_types::Folder = serde_json::from_str(&std::fs::read_to_string(&json_file).expect("Something went wrong reading the file"))?;

    writeln!(std::fs::File::create(cwd().join(format!("produced/{}.mpa", "ExampleProject")))?, "{}", serde_json::to_string(&script_project)?)?;
